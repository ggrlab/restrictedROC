...
wilcox_timepoints_fun <- function(data_named_timepoints,
                                  verbose = TRUE,
                                  save_intermediate_file = "intermediate_data/res_rroc_test.RDS",
                                  ...) {
    return(
...
li_all_data_wtest_unpaired <- lapply(
    li_all_data_measurements,
    wilcox_timepoints_fun,
    save_intermediate_file = file.path(outdir, "res_w_test_unpaired.qs"),
    paired = FALSE
)

li_all_data_wtest_paired <- lapply(
    li_all_data_measurements,
    wilcox_timepoints_fun,
    save_intermediate_file = file.path(outdir, "res_w_test_paired.qs"),
    paired = TRUE
)
```
...
```{r, Plot all tests}
all_tests <- list(
    "ttest_paired" = li_all_data_ttests_paired,
    "ttest_unpaired" = li_all_data_ttests_unpaired,
    "wilcox_paired" = li_all_data_wtest_paired,
    "wilcox_unpaired" = li_all_data_wtest_unpaired
    # , "rroc" = li_all_data_rroc
)
all_tests[[1]][[1]][[1]]
qvals <- Rvarious::unlist_get_element(all_tests, which.element = "q.value")
pvals <- Rvarious::unlist_get_element(all_tests, which.element = "p.value")
avg_fc <- Rvarious::unlist_get_element(all_tests, which.element = "avg.FC")
feature <- Rvarious::unlist_get_element(all_tests, which.element = "feature")

dplyr::full_join(feature, pvals, by = c("L0", "L1", "L2")) |> tibble::as_tibble()

f_p_fc <- cbind(feature, pvals[, "p.value", drop = FALSE], avg_fc[, "avg.FC", drop = FALSE]) |> tibble::as_tibble()

library(ggplot2)
pdf("removeme.pdf", width = 15, height = 10)
for (study in unique(f_p_fc[["L1"]])) {
    data_per_study <- f_p_fc |> dplyr::filter(L1 == study)
    print(
        ggplot(
            data_per_study,
            aes(x = avg.FC, y = -log10(p.value), label = feature)
        ) +
    ...
    # Make p.adjust per group
    p_corrected <- data_per_study |>
        dplyr::group_by(L0, L2) |>
        dplyr::mutate(q.value = p.adjust(p.value, method = "BH")) |>
        dplyr::ungroup()
    print(
        ggplot(
            p_corrected,
            aes(x = avg.FC, y = -log10(q.value), label = feature)
        ) +
            geom_vline(xintercept = 0, linetype = "dashed") +
            geom_point() +
            xlab("Average fold change") +
...
pdf(file.path(outdir, "fig2_global_vs_restriction.pdf"))
for (study_tp1_tp2_x in names(named_figs)) {
    study_tp1_tp2 <- named_figs[[study_tp1_tp2_x]]
    study_x <- study_tp1_tp2[[1]]
    tp1 <- study_tp1_tp2[[2]]
    tp2 <- study_tp1_tp2[[3]]
    data_part <- li_all_data_rroc[[study_x]][[paste0(tp1, "_", tp2)]]
    axis_limits <- c(min(c(1e-5, min(data_part[, c("max_p.value", "global_p.value")]))), 1)
    print(
        ggplot(
            data_part,
            aes(x = global_q.value, y = max_q.value, label = feature)
        ) +
            ggtitle(study_tp1_tp2_x, subtitle = paste0("Permutation test on AUC, global")) +
            geom_point() +
            ggrepel::geom_text_repel(
                data = subset(
                    data_part,
                    global_p.value > 0.05 & max_p.value < 0.05
                ),
                aes(label = feature),
