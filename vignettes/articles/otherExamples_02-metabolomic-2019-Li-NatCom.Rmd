...
for (n_permutations in c(25000)) {
    ...
    li_all_data_rroc <- sapply(
        ...
        function(study_x) {
            resfile <- paste0("intermediate_data/res_rroc_test_", n_permutations / 1000, "k_", study_x, ".qrds")
...
pdf("removeme.pdf", width = 4, height = 15)
...
pdf("removeme_2.pdf", height = 15, width = 10)
    ...
    for (study_tp1_tp2_x in names(named_figs)) {
        ...
        if (!feature_x %in% names(li_all_data_measurements[[study_x]][[tp1]])) {
            ...
            all_studies_plots[[study_tp1_tp2_x]] <- restrictedROC::plot_density_rROC_empirical(
                ...
                xmin = min(data_tp1, data_tp2),
                xmax = max(data_tp1, data_tp2)
            )[["plots"]]
        }
    }
