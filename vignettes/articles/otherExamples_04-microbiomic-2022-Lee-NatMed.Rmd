...
library(future)
plan(multisession)
for (n_permutations in c(10e3, 25e3, 50e3)) {
...
only_species <- all_taxa_results_summary[["paper_species_preprocessed"]]
only_species[["qval.twoside.max"]] <- p.adjust(only_species[["pval.twoside.max"]], method = "BH")
only_species[["qval.twoside.global"]] <- p.adjust(only_species[["pval.twoside.global"]], method = "BH")
# rroc_lee <- qs::qread(file.path(outdir, "rroc_pfs12_v2_10000.qs"))
# s_lee <- summary(rroc_lee)
# s_lee[["qval.twoside.max"]] <- p.adjust(s_lee[["pval.twoside.max"]], method = "BH")
# s_lee[["qval.twoside.global"]] <- p.adjust(s_lee[["pval.twoside.global"]], method = "BH")

# s_lee |>
#     dplyr::arrange(qval.twoside.max) |>
#     dplyr::select(level_1, level_2, qval.twoside.max, qval.twoside.global)
# lee_significant_restricted <- s_lee |>
#     dplyr::filter(qval.twoside.max < 0.05)
# lee_significant_restricted_additional <- s_lee |>
...
only_species |>
    dplyr::arrange(qval.twoside.max) |>
    dplyr::select(level_2, qval.twoside.max, qval.twoside.global)
library(ggplot2)
pval_cutoff <- .1
pdf("removeme.pdf", width = 12, height = 10)
print(
...
print(
    ggplot(only_species, aes(x = pval.twoside.max, y = pval.twoside.global, col = taxonomy)) +
        geom_point() +
        geom_vline(xintercept = pval_cutoff, linetype = "dashed") +
        geom_hline(yintercept = pval_cutoff, linetype = "dashed") +
        scale_x_log10() +
...
print(
    ggplot(only_species, aes(x = qval.twoside.max, y = qval.twoside.global, col = taxonomy)) +
        geom_point() +
        geom_vline(xintercept = pval_cutoff, linetype = "dashed") +
        geom_hline(yintercept = pval_cutoff, linetype = "dashed") +
        scale_x_log10() +
...
found_plain <- c("Firmicutes_Clostridia_Eubacteriales_Lachnospiraceae_NA_.Eubacterium..rectale")

pdf("removeme_density.pdf", width=9, height=8)
print(
