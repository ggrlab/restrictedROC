...
harel_data <- readxl::read_excel("intermediate_data/harel2019-s1.xlsx", sheet = "S1B", skip = 1, na = "NaN")
harel_exprs <- as.matrix(harel_data[, 1:116])
harel_fdata <- harel_data[, 117:ncol(harel_data)]
rownames(harel_exprs) <- harel_data[["T: Gene names"]]
...
```
```{r, merge features with same gene name}
harel_exprs_nodup <- harel_exprs[!duplicated(rownames(harel_exprs)), ]
for(dup_x in unique(rownames(harel_exprs)[duplicated(rownames(harel_exprs))])){
    harel_exprs_nodup <- rbind(
        harel_exprs_nodup, 
        apply(harel_exprs[which(rownames(harel_exprs) == dup_x), ], 2, median, na.rm=TRUE)
    )
    rownames(harel_exprs_nodup)[nrow(harel_exprs_nodup)] <- dup_x
}
```
```{r, filter 70% valid for TIL and aPD1}
harel_exprs_til <- harel_exprs_nodup[, grepl("^TIL", colnames(harel_exprs_nodup))]
harel_exprs_aPD1 <- harel_exprs_nodup[, grepl("^PD1", colnames(harel_exprs_nodup))]


harel_exprs_f70 <- harel_exprs_nodup[apply(harel_exprs_nodup, 1, function(x) sum(!is.na(x))) < 0.7 * ncol(harel_exprs_nodup), ]
harel_exprs_til_f70 <- harel_exprs_til[apply(harel_exprs_til, 1, function(x) sum(!is.na(x))) < 0.7 * ncol(harel_exprs_til), ]
harel_exprs_aPD1_f70 <- harel_exprs_aPD1[apply(harel_exprs_aPD1, 1, function(x) sum(!is.na(x))) < 0.7 * ncol(harel_exprs_aPD1), ]

dim(harel_exprs_til_f70)
dim(harel_exprs_aPD1_f70)
dim(harel_exprs_f70)
dim(harel_exprs_til_f70[!is.na(rownames(harel_exprs_til_f70)), ])
dim(harel_exprs_aPD1_f70[!is.na(rownames(harel_exprs_aPD1_f70)), ])
dim(harel_exprs_f70[!is.na(rownames(harel_exprs_f70)), ])
```
...
harel_til <- readxl::read_excel("intermediate_data/harel2019-s1.xlsx", sheet = "S1C", skip = 1, na = "NaN")
harel_aPD1 <- readxl::read_excel("intermediate_data/harel2019-s1.xlsx", sheet = "S1D", skip = 1, na = "NaN")
