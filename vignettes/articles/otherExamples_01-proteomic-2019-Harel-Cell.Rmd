...
for (feature_outcome in list(
    ...
    rroc_single_result <- restrictedROC::plot_density_rROC_empirical(
        values_grouped = grouped_vals,
        positive_label = "responder"
    )
    p1 <- rroc_single_result[["plots"]]
    ...
    grouped_vals_df <- rbind(
        tidyr::pivot_longer(tibble::as_tibble(grouped_vals[1]), everything()),
        tidyr::pivot_longer(tibble::as_tibble(grouped_vals[2]), everything())
    )
    rroc_single <- rroc_single_result[["single_rROC"]]
    predicted <- restrictedROC:::predict.restrictedROC(
        rroc_single,
        newdata = grouped_vals_df,
        newdata_predictor_column = 2, newdata_response_column = 1,
        pred_high_label = "responder",
        pred_low_label = "nonresponder"
    )
    perf_df_global <- tibble::tibble(
        ...
        "inf_range.min" = -Inf,
        "inf_range.max" = Inf,
        "high.median" = names(which.max(lapply(grouped_vals, median)))
    )
    ...
    print(patchwork::wrap_elements(
        ...
        gridExtra::tableGrob(
            rroc_res[[outcome_x]] |>
                dplyr::filter(level_2 == feature_x),
            theme = gridExtra::ttheme_default(base_size = 2)
        )
    ) + patchwork::wrap_elements(
        gridExtra::tableGrob(
