...
source("vignettes/articles/utils.R")
int_dir <- "intermediate_data/2019-harel/rroc_intermediate"
dir.create(int_dir, recursive = TRUE)
library(future)
plan(multisession)
rroc_res <- sapply(names(data_mats), function(x_name) {
    x <- data_mats[[x_name]]
    x_noNArownames <- x[!is.na(rownames(x)), ]
    rroc_permutation_many_features(
        x = x_noNArownames,
        response = outcome_noSD[colnames(x)],
        save_intermediate_file = file.path(int_dir, paste0("harel_nPERM", n_permutations, "_", x_name, ".qrds")),
...
# 10k permutations took 14s for one feature.
# There are 4620 (aPD1) and 4588 (TIL) features
# Therefore that takes 35h.
rroc_res <- sapply(names(data_mats), function(x_name) {
    qs::qread(file.path(int_dir, paste0("harel_nPERM", n_permutations, "_", x_name, ".qrds")))
}, USE.NAMES = TRUE, simplify = FALSE)
```
...
library(ggplot2)
pdf("intermediate_data/2019-harel/global_max_pval.pdf")
for (name_x in names(rroc_res)) {
    p1 <- ggplot(rroc_res[[name_x]], aes(x = global_p.value, y = max_p.value))
    p2 <- ggplot(rroc_res[[name_x]], aes(x = global_q.value, y = max_q.value))
        ...
        print(
            plot_x +
                ...
                ggrepel::geom_text_repel(
                    data = subset(
                        rroc_res[[name_x]],
                        global_q.value > 0.05 & max_q.value < 0.05
                    ),
                    aes(label = feature),
                    size = 1.5, col = "darkgreen",
                    min.segment.length = 0,
...
pdf("intermediate_data/2019-harel/rroc_significant.pdf", height = 8)
for (feature_outcome in list(
    ...
    p1 <- restrictedROC::plot_density_rROC_empirical(
    ...
    print(patchwork::wrap_elements(
        ...
        gridExtra::tableGrob(
            rroc_res[[outcome_x]] |> dplyr::filter(feature == feature_x),
            theme = gridExtra::ttheme_default(base_size = 8)
        )
    ) + patchwork::wrap_elements(
        gridExtra::tableGrob(
            harel_ttests_q[[outcome_x]] |> dplyr::filter(gene == feature_x),
            theme = gridExtra::ttheme_default(base_size = 8)
        )
    ) +
        patchwork::plot_layout(nrow = 3, height = c(.8, .1, .1)))
}
dev.off()
```
