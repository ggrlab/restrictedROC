...
``` r
perf_ROC(response, predictor, how = "pROC", positive_label = NULL, ...)
```
...
- how:
  ...
  Previously, I allowed "pROC" and "ROCR", but now I only allow "pROC"
  after it is way faster.
...
List of perf: Result of
[`perf_pROC()`](https://ggrlab.github.io/restrictedROC/reference/perf_pROC.md),
comes from [`pROC::coords()`](https://rdrr.io/pkg/pROC/man/coords.html)
auc: Area under the ROC curve, comes from
[`pROC::auc()`](https://rdrr.io/pkg/pROC/man/auc.html) positive_label:
Label of the positive class auc_variance: Variance of the AUC under
Nullhypothesis: \$\$\frac{n\_{positives} + n\_{negatives} + 1}{12\cdot
n\_{positives} \cdot n\_{negatives}}\$\$ rzAUC: restricted standardized
AUC, combines the calculated AUC with the variance.
